...
driver.find_element_by_xpath(day_button).click()
driver.find_element_by_xpath(yesterday_button).click()  #选择昨天
time.sleep(2)
...
actimes2 = [actimes2[i].text for i in range(len(actimes2))]
...
cur = datetime.date.today() + datetime.timedelta(-1)

output_file = './output/大兴机场_航班时间'
timelist = str(cur.year) + str(cur.month) + str(cur.day)  
output_file = output_file + timelist + 'yesterday_dep_only' + '.xls'
